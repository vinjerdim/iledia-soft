...
/* ============================================================
   app.js — perakitan materi 2.2  (DIMUAT PALING AKHIR)
   ============================================================
...
var lesson = Engine.createLesson({
  ...
  createState: function () {
    return {
      ...
      /* Tahap 2 — Stimulasi */
      stimulasi: {
        opened: mapFrom(DATA.stimulasi.gudang, function () {
          return { seen: false };
        }),
        picked: [],
        checked: false,
      ...
      olah: {
        produk: { assignments: {}, selectedId: null, checked: false, correct: 0 },
        ciri: { assignments: {}, selectedId: null, checked: false, correct: 0 }
      },
...
assertUniqueIds(DATA.stimulasi.gudang, 'stimulasi.gudang');
assertUniqueIds(DATA.stimulasi.amatan, 'stimulasi.amatan');
...
assertUniqueIds(DATA.olah.produkColumns, 'olah.produkColumns');
assertUniqueIds(DATA.olah.produkChips, 'olah.produkChips');
assertUniqueIds(DATA.olah.ciriColumns, 'olah.ciriColumns');
assertUniqueIds(DATA.olah.ciriChips, 'olah.ciriChips');
assertUniqueIds(DATA.uji.jenis, 'uji.jenis');
assertUniqueIds(DATA.uji.cases, 'uji.cases');
DATA.uji.cases.forEach(function (c) {
  assertUniqueIds(c.options.map(function (id) { return { id: id }; }), 'uji.' + c.id + '.options');
  if (c.options.indexOf(c.correct) === -1) console.error('uji.' + c.id + ': jawaban benar tidak ada di pilihan.');
  c.options.forEach(function (id) {
    if (!findById(DATA.uji.jenis, id)) console.error('uji.' + c.id + ': jenis tidak dikenal: ' + id);
  });
});
...
assertUniqueIds(
  DATA.olah.produkChips.concat(DATA.olah.ciriChips, DATA.olah.produkColumns, DATA.olah.ciriColumns),
