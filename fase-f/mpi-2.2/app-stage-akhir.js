...
function renderRefleksi(container) {
  ...
  /* Mengingat kembali rekomendasi saat menjadi konsultan. Boleh saja
  ...
  var recallItems = DATA.uji.cases
    .map(function (c) {
      ...
      var opt = findById(DATA.uji.jenis, u.prediction);
      if (!opt) return '';
      return (
        ...
        '<p class="recall-item__guess">Rekomendasimu: "' + esc(opt.label) + '"</p>' +
...
function renderSelesai(container) {
  ...
  var labelSkor = {
    stimulasi: 'Pengamatan enam gudang',
    masalah: 'Rumusan masalah',
    konsep: 'Menjodohkan jenis & ciri',
    olah: 'Lemari arsip DBMS',
    uji: 'Konsultan DBMS',
  ...
  var bandingRows = d.banding.map(function (row) {
    return row.map(function (cell, i) {
      return i === 0 ? '<strong>' + esc(cell) + '</strong>' : esc(cell);
    });
  });
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<h3>' + esc(d.bandingLabel) + '</h3>' +
    miniTable(d.bandingHeaders, bandingRows) +
    '</div>' +
