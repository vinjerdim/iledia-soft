...
/* ============================================================
   app.js — perakitan materi 1.4  (DIMUAT PALING AKHIR)
   ============================================================
...
var STAGE_DEFS = [
  ...
  { id: 'bekal', label: 'Bekal Konsep', render: 'renderBekal' },
  { id: 'telusur', label: 'Telusur Temuan', render: 'renderTelusur' },
  { id: 'saring', label: 'Saring Jenis Relasi', render: 'renderSaring' },
  { id: 'klasifikasi', label: 'Klasifikasi FK & Penghubung', render: 'renderKlasifikasi' },
...
var lesson = Engine.createLesson({
  ...
  createState: function () {
    return {
      ...
      /* Tahap 4 — Telusur petunjuk lapangan */
      telusur: {
        marked: {},
        revealed: false,
        lastFeedback: null
      ...
      /* Tahap 6 — Klasifikasikan FK & tabel penghubung */
      klasifikasi: {
        fkAnswers: {},
        checkedFk: false,
        usulan: [],
...
assertUniqueIds(DATA.entitas.list, 'entitas.list');
assertUniqueIds(DATA.entitas.attrs, 'entitas.attrs');
...
assertUniqueIds(DATA.saring.columns, 'saring.columns');
assertUniqueIds(DATA.klasifikasi.fkQuestions, 'klasifikasi.fkQuestions');
DATA.klasifikasi.fkQuestions.forEach(function (q) {
  assertUniqueIds(q.options, 'klasifikasi.' + q.id + '.options');
});
...
/* klasifikasi.fkQuestions berbagi id dengan lima dari enam pasangan
   saring.chips (semua kecuali pasangan Many-to-Many), supaya tahap 7
   (sajikan) bisa menggabungkan jenis relasi dan jawaban FK-nya lewat
   id yang sama. Jaga agar keduanya tetap cocok. */
DATA.klasifikasi.fkQuestions.forEach(function (q) {
  if (!findById(DATA.saring.chips, q.id)) {
    console.error('klasifikasi.fkQuestions id tidak cocok dengan saring.chips: ' + q.id);
  }
});
