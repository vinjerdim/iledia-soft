...
/* Ringkasan relasi final, dipakai tahap 7 (sajikan) dan tahap 10
   (selesai — lihat app-stage-akhir.js). Diturunkan langsung dari
   DATA (kunci jawaban), bukan dari State, karena tahap ini hanya
   terbuka setelah tahap 5 dan 6 dijawab tepat semua. */
function relasiRows() {
  return DATA.saring.chips.map(function (c) {
    var kol = findById(DATA.saring.columns, c.entityId);
    var parts = c.label.split(' — ');
    var fk;
    if (c.entityId === 'banyak-banyak') {
      var kolom = DATA.klasifikasi.usulan
        .filter(function (u) {
          return u.simpan;
        })
        .map(function (u) {
        ...
        .join(', ');
      fk = 'Tabel penghubung "' + DATA.klasifikasi.junctionTableName + '" (' + kolom + ')';
    } else {
      var q = findById(DATA.klasifikasi.fkQuestions, c.id);
      var opt = findById(q.options, q.correct);
      fk = opt.label;
    }
    return [esc(parts[0]), esc(kol.name), esc(parts[1]), esc(fk)];
  });
}
...
function renderSajikan(container) {
  ...
  container.innerHTML =
    ...
    '<p>' + esc(d.kartuNote) + '</p>' +
    miniTable(d.tableHeaders, relasiRows()) +
    '</div>' +
